...
final class ForgotPasswordScreen: UIView {
    ...
    private func configLabel() {
        titleLabel.setDTO(with: .init(text: "Informe o seu email aqui para redefinar a senha",
                                      textcolor: DSColors.textColorTitle,
    ...
    private func configButton() {
        ...
        enterButton.onClick { self.delegate?.actionEnterButton() }
        
        backButton.setDTO(.init(title: "Back <-", isEnable: true, font: DSFonts.subtitleSemibold16))
        backButton.onClick { self.delegate?.actionBackButton() }
    }
}
